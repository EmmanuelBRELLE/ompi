                                  ...
                                  mca_coll_base_module_t * module)
{
    ...
    if (w_rank == root) {
        ...
        if (han_module->is_mapbycore) {
            ...
            reorder_buf_start = (char *)sbuf;
        } else {
            OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
                                 "[%d]: Han scatter: needs reordering: ", w_rank));
            ptrdiff_t sgap = 0;
            ptrdiff_t ssize = opal_datatype_span(&sdtype->super,
                                                 (int64_t)scount * w_size,
                                                 &sgap);
            reorder_buf = (char *)malloc(ssize);
            /* sgap is the size of unused space at the start of the datatype */
            reorder_buf_start = reorder_buf - sgap;
            /* reorder data into reorder buffer */
            ompi_coll_han_reorder_scatter(sbuf,
                                          reorder_buf_start, scount, sdtype,
                                          comm, topo);
    ...
    if (low_rank == root_low_rank) {
        ...
        if (reorder_buf != NULL) {
            free(reorder_buf);
            reorder_buf_start = NULL;
            reorder_buf = NULL;
        }
    }

    /* 2. low scatter on nodes leaders */
    low_comm->c_coll->coll_scatter((char *)tmp_buf_start,
                     count,
                     dtype,
                     (char*)rbuf,
                     count,
...
/** Reorder before scatter operation, for unordered ranks
 *
 * This does the 'inverse' of reoder_gather (see src/dest shift).
...
ompi_coll_han_reorder_scatter(const void *sbuf,
   ...
   for (i=0; i<w_size; i++) {
           OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
                           "[%d]: Han reorder from %d to %d\n",
                           w_rank,
                           topo[i * topolevel + 1],
                           i * topolevel + 1));
           ptrdiff_t block_size = extent * (ptrdiff_t)count;
           ptrdiff_t src_shift = block_size * (ptrdiff_t)topo[i * topolevel + 1];
           ptrdiff_t dest_shift = block_size * i;
